...
const Slug = () => {
  ...
  return (
    <>
         <div className="w-fit mx-auto mt-10 "> <input onChange={(e)=>setSearchQuery(e.target.value)} value={searchQuery} type="text" className=" border-2  h-12 border-orange-500 outline-orange-600 px-3  w-[55vw] text-lg" /> <button onClick={()=>Router.push(`/assignments/tags/${searchQuery}`)} className="w-[5vw] border-2 border-orange-500  h-12">search</button>   </div>s
      {loader &&
        [1, 2, 3].map((l) => (
          <AssignMentLoader
            key={l}
        ...
        {assignment && assignment.length == 0 && !loader && (
          <div className="flex h-[10vh] w-[100vw] overflow-hidden justify-center items-center">
            <div className="  text-4xl  rounded-md font-semibold text-center">No Assignment found with the tag {`'${Slug}'`}</div>
          </div>
        )}
      {assignment &&
